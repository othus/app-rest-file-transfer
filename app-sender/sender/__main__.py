    ...
    for file in os.listdir(settings.INPUT_DIR):
        if file.endswith(".json"):
            ...
            if status.is_new(file):
                ...
                encryptXML(file, file_prefix)
                uploadToServer(file, file_prefix)
...
"""
Auxiliary function to make the call to util.encrypt_xml and avoid redundant code.
:param file: Provide the path to the symmetric key
:param file_prefix: File name (without file extension)
:return: None
"""
def encryptXML(file, file_prefix):
    utils.encrypt_xml(status,
                        enc,
                        "{0}/{1}.xml".format(settings.INPUT_DIR, file_prefix),
                        file)

def uploadToServer(file, file_prefix):
